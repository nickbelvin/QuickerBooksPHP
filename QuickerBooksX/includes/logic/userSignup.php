...
if (isset($_POST['signup_btn'])) {
	...
	if (count($errors) === 0) {
		...
		$query = "INSERT INTO users SET role_id=?, token=?, username=?, fname=?, lname=?, email=?, phone=?, password=?, profile_picture=?, dob=?, created_at=?";
		$stmt = $conn->prepare($query);
		$stmt->bind_param('issssssssss', $role_id, $token, $username, $fname, $lname, $email, $phone, $password, $profile_picture, $dob, $created_at);
		$result = $stmt->execute();
		if ($result) {
        ...
        if ( ($verified == 1)) {
            ...
            if ($user['locked'] == 0) {
			   ...
			   $sql = "UPDATE users SET login_fail_count = login_fail_count + 1 WHERE username = '$username'";
				$stmt = $conn->prepare($sql);
				$stmt->execute();
				$_SESSION['error_msg'] = nl2br("Account is locked. \nReset password or contact your administrator for access");
            }
