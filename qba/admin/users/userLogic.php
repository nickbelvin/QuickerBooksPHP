...
global $conn, $errors, $username, $role_id, $email, $fname, $lname, $phone, $dob, $status, $isEditing, $isSuspended;
...
if (count($errors) === 0) {
  ...
  $sql = "UPDATE users SET role_id=?, username=?, fname=?, lname=?,  email=?, status=?, phone=?, password=?,  profile_picture=?, dob=? WHERE id=?";
  $result = modifyRecord($sql, 'isssssssssi', [$role_id, $username, $fname, $lname, $email, $status, $phone, $password, $profile_picture, $dob, $user_id]);
...
function saveUser(){
  global $conn, $errors, $username, $role_id, $email, $fname, $lname, $phone, $dob, $status, $isEditing;
...
$sql = "SELECT r.name as role, u.password_expiry, u.id, u.role_id, u.username, u.email, u.phone, u.dob, u.fname, u.lname, u.status
        FROM users u
        LEFT JOIN roles r ON u.role_id=r.id
        WHERE role_id IS NOT NULL ";

$users = getMultipleRecords($sql, 'i');
return $users;
}

function editUser($user_id){
global $conn, $user_id, $role_id, $username, $phone, $fname, $lname, $dob, $email, $status, $isEditing, $profile_picture;
